...
df_filtered = pd.concat([df_vote_count_filter, df_high_revenue]).drop_duplicates(subset=['id'])
...
print(f"Filtered dataset size (top 50k by vote_count OR revenue > $1M, duplicates removed, 1930+ only): {len(df_filtered)}")
...
for year in sorted(df_filtered['year'].unique()):
    ...
    if len(year_data) > 0:
        ...
        print(f"Year {year}: {len(year_top)} movies (max revenue: ${year_top['revenue'].max():,})")
...
df_top_by_year = df_top_by_year.sort_values(['year', 'revenue'], ascending=[True, False])
...
chunk_size = 10000
num_chunks = len(df_top_by_year) // chunk_size + (1 if len(df_top_by_year) % chunk_size != 0 else 0)
